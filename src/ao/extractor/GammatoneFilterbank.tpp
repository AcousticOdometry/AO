...
template <typename T>
void GammatoneFilterbank<T>::GammatoneFilter::compute(
    ...
    for (int i = 0; i < input.size(); i++) {
        ...
        env[i] = sqrt(u0r * u0r + u0i * u0i) * this->gain;
...
template <typename T>
void GammatoneFilterbank<T>::GammatoneFilter::compute_with_temporal_integration(
    ...
    // than simply sampling it.
    response =
        intgain * std::accumulate(env.begin(), env.end(), 0.0) / env.size();
}

template <typename T>
std::vector<typename Filterbank<T>::Filter&>
GammatoneFilterbank<T>::make_filters(
    ...
    // Initialize output
    std::vector<GammatoneFilterbank<T>::GammatoneFilter> filters;
    ...
    for (int i = 0; i < num_filters; i++) {
        ...
        // TODO help on this
        // ? why in ERB scale ?
        const T tptbw = tpt * ERB(cf) * this->bandwidth_correction;
        ...
        const std::array<T, 5> a = {
            ...
            _a * _a};
        // filters.push_back(GammatoneFilter(
        //     cf, std::cos(tpt * cf), std::sin(tpt * cf), gain, a));
    }
    return filters;
}
