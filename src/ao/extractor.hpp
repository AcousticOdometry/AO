...
// TODO
template <typename T> class Filterbank : public Extractor<T> {
    public:
    class Filter {
        public:
         ...
         */
        virtual void
        compute(const std::vector<T>& input, T& response) const = 0;
    ...
    public:
    /**
     * @brief Construct a new Filterbank object
     *
    ...
    // TODO
    virtual std::vector<Filter&> make_filters(
        const T& low_Hz,
        const T& high_Hz,
        const size_t& num_filters,
        const int& sample_rate) const = 0;
};
...
//          * @return T Filter response.
//          */
//         T operator()(const std::vector<T>& input, const T& intdecay = 0)
//         const {
//             T feature;
...
//          */
//         void
//         compute(const std::vector<T>& input, T& response, const T& intdecay =
//         0)
//             const;
//     };
